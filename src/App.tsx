...
function App() {
  ...
  const [providerManager] = useState(() => {
    ...
    // Register mock provider for development/testing (can be disabled in production)
    // const mockProvider = new MockNJTransitProvider();
  ...
  useEffect(() => {
    if (followState.enabled && followState.trainId) {
      ...
      if (train?.locationHypothesis?.position) {
        setMapCenter([
          train.locationHypothesis.position.lat,
          train.locationHypothesis.position.lng,
        ]);
        setMapZoom(14);
      }
    }
